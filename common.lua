...
-- do not override this function without knowing what you are doing
function luamap.on_generated(vm, emin, emax, minp, maxp, seed)
	-- localize vars 
    ...
    for name,elements in pairs(noises_2d) do
		if not(maxp.y <= elements.ymin and minp.y >= elements.ymax) then
			noises_2d[name].nobj = noises_2d[name].nobj or core.get_perlin_map(noises_2d[name].np_vals, chulens2d)
	...
	for name,elements in pairs(noises_3d) do
		if not(maxp.y <= elements.ymin and minp.y >= elements.ymax) then
			noises_3d[name].nobj = noises_3d[name].nobj or core.get_perlin_map(noises_3d[name].np_vals, chulens3d)
